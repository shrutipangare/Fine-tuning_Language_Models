...
def get_args():
    ...
    parser.add_argument('--from_scratch', action='store_true', help="Train T5 model from scratch instead of fine-tuning")
    ...
    parser.add_argument('--freeze_encoder_layers', type=int, default=0,
                    help="Freeze bottom-k encoder layers of T5 (0 = no freezing)")
...
def train(args, model, train_loader, dev_loader, optimizer, scheduler):
    ...
    for epoch in range(args.max_n_epochs):
        print(f"Starting Epoch {epoch+1} of {args.max_n_epochs}") 
...
def eval_epoch(args, model, dev_loader, gt_sql_pth, model_sql_path, gt_record_path, model_record_path):
    '''
    Evaluation:
      - compute CE loss on dev (teacher forcing, like train)
      - generate SQL (beam search), save to model_sql_path and model_record_path
      - compute metrics with compute_metrics()
      - compute syntax error rate from DB error messages
    ...
    # generation loop to produce SQL strings
    gen_sql = []
    with torch.no_grad():
        for encoder_input, encoder_mask, _, _, _ in tqdm(dev_loader):
            encoder_input = encoder_input.to(DEVICE)
            encoder_mask  = encoder_mask.to(DEVICE)
            ...
            outputs = model.generate(
                input_ids=encoder_input,
                attention_mask=encoder_mask,
                max_length=256,              # match MAX_TGT_LEN
                num_beams=4,
                length_penalty=0.6,     
                early_stopping=True,
            )
            ...
            gen_sql.extend([s.strip() for s in decoded])

    #  save queries and records using utils.save_queries_and_records
    os.makedirs(os.path.dirname(model_sql_path), exist_ok=True)
    save_queries_and_records(gen_sql, model_sql_path, model_record_path)

    # compute metrics (note the order of returned values!)
    sql_em, record_em, record_f1, model_error_msgs = compute_metrics(
        gt_sql_pth, model_sql_path, gt_record_path, model_record_path
    )

    # syntax error rate (non-empty error_msg means SQL failed)
...
def test_inference(args, model, test_loader, model_sql_path, model_record_path):
    '''
    Generate SQL for the test split and save both queries and DB records.
    No loss or metrics here (Gradescope/leaderboard will score test).
    '''
    model.eval()
    gen_sql = []
    with torch.no_grad():
        # test_collate_fn returns: encoder_ids, encoder_mask, initial_decoder_inputs
    ...
    os.makedirs(os.path.dirname(model_sql_path), exist_ok=True)
    save_queries_and_records(gen_sql, model_sql_path, model_record_path)
    print(f"[TEST] Saved SQL -> {model_sql_path}")
    print(f"[TEST] Saved predicted records -> {model_record_path}")
...
def main():
    ...
    from load_data import PAD_IDX  # at the top of file if not already

    # Ensure T5 config matches our tokenizer behavior
...
'''def main():
    ...
    # If from_scratch is True, set finetune to False
    ...
    if args.eval:
        ...
        # Perform evaluation on dev and test sets
        ...
        gt_sql_path = os.path.join(f'data/dev.sql')
        gt_record_path = os.path.join(f'records/ground_truth_dev.pkl')
        model_sql_path = os.path.join(f'results/t5_ft_experiment_dev.sql')
        model_record_path = os.path.join(f'records/t5_ft_experiment_dev.pkl')
