...
def create_prompt(sentence, k, train_x, train_y, schema_text):
    """
    Builds a zero/few-shot prompt for the LLM.
    It includes the database schema, k training examples, and the new question.
    """
    header = (
        "You are an expert data analyst translating natural language to SQL queries.\n"
        "Use the database schema and the examples below to output ONLY the SQL query.\n\n"
        "DATABASE SCHEMA:\n"
        f"{schema_text}\n"
        "----\n"
    )
    ...
    # Few-shot examples
    examples = ""
    for i in range(min(k, len(train_x))):
        examples += f"NL: {train_x[i]}\nSQL: {train_y[i]}\n\n"

    # Add current test example
    task = f"NL: {sentence.strip()}\nSQL:"
...
def exp_kshot(tokenizer, model, inputs, k, train_x, train_y, schema_text):
    """
    Performs k-shot prompting on each input and returns raw text + extracted SQL queries.
    """
    raw_outputs, extracted_queries = [], []
    model.eval()

    for i, sentence in tqdm(enumerate(inputs), total=len(inputs)):
        prompt = create_prompt(sentence, k, train_x, train_y, schema_text)
...
def eval_outputs(extracted_queries, gt_sql_path, model_sql_path, gt_record_path, model_record_path):
    """
    Save generated SQL queries, compute records and metrics.
    """
    os.makedirs(os.path.dirname(model_sql_path), exist_ok=True)
    os.makedirs(os.path.dirname(model_record_path), exist_ok=True)

    # Write generated SQLs to file
    with open(model_sql_path, "w") as f:
        for q in extracted_queries:
            f.write((q or "").strip() + "\n")

    # Compute predicted records from SQL
    save_queries_and_records(model_sql_path, model_record_path)

    # Compare against ground truth
    record_f1, record_em, sql_em = compute_metrics(
        gt_sql_path, model_sql_path, gt_record_path, model_record_path
    )

    # Error analysis
    empties = sum(1 for q in extracted_queries if not q.strip())
    error_rate = empties / max(1, len(extracted_queries))
    model_error_msgs = {"empty_generations": empties}
...
def main():
    ...
    schema_text = read_schema("flight_database.schema")
    ...
    for eval_split in ["dev", "test"]:
        ...
        raw_outputs, extracted_queries = exp_kshot(tokenizer, model, eval_x, shot, train_x, train_y, schema_text)
        ...
        # )
        sql_em, record_em, record_f1, model_error_msgs, error_rate = eval_outputs(
        extracted_queries, gt_sql_path, model_sql_path, gt_record_path, model_record_path
        )
