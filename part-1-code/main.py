...
from utils import *
import os
from datasets import concatenate_datasets  # Add this import
...
# Core evaluation function output_dir
...
def create_augmented_dataloader(args, dataset):
    ...
    # Create a transformed version of the training dataset
    transformed_train_dataset = original_train_dataset.map(
        custom_transform, 
        load_from_cache_file=False
    ) 
    
    # Randomly select 5000 transformed examples
    num_augment = 5000
    transformed_subset = transformed_train_dataset.shuffle(seed=42).select(range(min(num_augment, len(transformed_train_dataset))))
    
    # Combine original and transformed datasets
    augmented_dataset = concatenate_datasets([original_train_dataset, transformed_subset])
    
    # Tokenize the augmented dataset
    augmented_tokenized_dataset = augmented_dataset.map(
        tokenize_function, 
        batched=True, 
        load_from_cache_file=False
    )
